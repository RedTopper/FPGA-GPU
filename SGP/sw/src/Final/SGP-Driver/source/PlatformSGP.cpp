...
namespace SuperHaxagon {
	PlatformSGP::PlatformSGP(const Dbg dbg): Platform(dbg) {
		...
		PlatformSGP::message(Dbg::INFO, "platform", "booting");
		PlatformSGP::message(Dbg::INFO, "platform", Mix_GetError());
		...
		if (!_display) {
			PlatformSGP::message(Dbg::FATAL, "display", "error " + std::to_string(eglGetError()));
			return;
		}

		if (!initEGL()) {
			PlatformSGP::message(Dbg::FATAL, "egl", "there was a fatal error creating an opengl context");
			return;
		}
		...
		PlatformSGP::message(Dbg::INFO, "platform",  "opengl ok");
	}
	...
	Vec2f PlatformSGP::getScreenDim() const {
		return Vec2f{static_cast<float>(_width), static_cast<float>(_height)};
	}

	void PlatformSGP::screenBegin() {
		_z = 0.0f;
	...
	void PlatformSGP::screenFinalize() {
	...
	void PlatformSGP::drawPoly(const Color& color, const std::vector<Vec2f>& points) {
	...
	float PlatformSGP::getAndIncrementZ() {
	...
	template<class T>
	void PlatformSGP::render(std::deque<std::shared_ptr<RenderTarget<T>>> targets, bool transparent) {
